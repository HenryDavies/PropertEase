...
function saveProperty(listing, callback) {
  Property.create(listing, (err) => {
    ...
    console.log(`listing ${listing.listing_id} saved`);
    callback();
  });
}
...
function parseAndSave(listing, callback) {
  if (listing.floor_plan !== [] && listing.floor_plan) {
    download(listing.floor_plan[0], path.join(__dirname, `../images/${listing.listing_id}.png`), () => {
      ...
      fs.unlink(path.join(__dirname, `../images/${listing.listing_id}.png`), () => console.log('file deleted'));
    });
  } else saveProperty(listing,callback);
}
...
//   saveSqFtData(shortArray);
// });

// function saveSqFtData(array) {
//   console.log('reached sq ft data function');
//   // console.log(array);
//   async.eachLimit(array, PARALLEL_LIMIT, editProperty, function(err) {
//     console.log('reached');
...
//   console.log('reached edit property function');
//   if (listing.price == 0) {
...
//       }
//     });
//   } else if (listing.floor_plan !== [] && listing.floor_plan) {
//     download(listing.floor_plan[0], path.join(__dirname, `../images/${listing.listing_id}.png`), () => {
//       vision.detectText(path.join(__dirname, `../images/${listing.listing_id}.png`), (err, text) => {
//         if (err) {
//           console.log('cloud vision error:', err);
